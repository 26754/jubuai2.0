...
export async function migrateToProjectStorage(): Promise<void> {
  ...
  try {
    ...
    const projectStoreRaw = await fileStorage.getItem('jubuai-project-store');
    if (!projectStoreRaw) {
    ...
    await migrateRecordStore('jubuai-script-store', 'script', projectIds);
    await migrateRecordStore('jubuai-director-store', 'director', projectIds);

    // 3. Migrate flat-array stores (media, characters, scenes)
    await migrateFlatStore('jubuai-media-store', 'media', projectIds, {
    ...
    await migrateFlatStore('jubuai-character-library', 'characters', projectIds, {
      arrayKeys: ['characters', 'folders'],
      projectIdField: 'projectId',
      sharedFilter: (item: any) => !item.projectId,
    });

    await migrateFlatStore('jubuai-scene-store', 'scenes', projectIds, {
...
async function migrateTimelineStore(activeProjectId: string): Promise<void> {
  const raw = await fileStorage.getItem('jubuai-timeline-store');
  if (!raw) return;
...
export async function recoverFromLegacy(): Promise<void> {
  ...
  try {
    ...
    await recoverRecordStore('jubuai-script-store', 'script', isScriptDataRich);
    await recoverRecordStore('jubuai-director-store', 'director', isDirectorDataRich);
